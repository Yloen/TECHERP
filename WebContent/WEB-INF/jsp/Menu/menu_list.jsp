...
<body>
	...
	<span class="c-gray en">&gt;</span> 菜单管理 <span class="c-gray en">&gt;</span>
	菜单列表 <a class="btn btn-success radius r"
		...
		<div class="text-c">
			...
			<form id="search_name">
				<input type="text" class="input-text" style="width: 250px"
					placeholder="输入用户名称" id="MenuName" name="MenuName">
			...
			<table id="datalist"
				...
				<tbody>
					...
					<c:forEach items="${sessionScope.menulist}" var="menu">
						<tr class="text-c">
							<td><input type="checkbox" value="${menu.getMenuId()}"
								name="ckbok"></td>
							<td>${menu.getMenuId()}</td>
							<td><u style="cursor: pointer" class="text-primary"
								onclick="member_show('${menu.getMenuName()}','${pageContext.request.contextPath}/Menu/Menu_show?MenuId=${menu.getMenuId()}','10001','360','400')">${menu.getMenuName()}</u></td>
							
							<td>${menu.getMenuValue()}</td>
							<td>${menu.getMenuSrc()}</td>
							<td>
							<img width="30px" height="30px"
							src="${pageContext.request.contextPath}/images/${menu.getPicSrc()}"></td>
							
							<td>${menu.getCategoryName()}</td>
							
							<td class="td-manage"><a style="text-decoration: none"
								onClick="member_stop(this,'10001')" href="javascript:;"
								title="停用"><i class="Hui-iconfont">&#xe631;</i></a> <a
								title="编辑" href="javascript:;"
								onclick="member_edit('编辑','${pageContext.request.contextPath}/Menu/Menu_edit?MenuId=${menu.getMenuId()}','4','','510')"
								class="ml-5" style="text-decoration: none"> <i
									class="Hui-iconfont">&#xe6df;</i>
							</a>  <a title="删除" href="javascript:;"
								onclick="member_del(this,'${menu.getMenuId()}')" class="ml-5"
  ...
  function member_del(obj, id) {
    layer.confirm('确认要删除吗？', function (index) {
      $.ajax({
        type: 'POST',
        url: '${pageContext.request.contextPath}/Menu/Menu_Delete?MenuId='+id,
        dataType: 'json',
        success: function (data) {
 ...
 function delect_batch(){ 
   ...
   console.log("选中值："+ ids);
     ...
     if (confirm("是否 删除?")){
       $.post ("${pageContext.request.contextPath}/Menu/Menu_DeleteBatch", {"ids":ids},
       function (data){
      if (data>0) {
         alert("删除成功");
          }else {
...
$("#search").click(function() {
	  $.ajax({
          ...
          url : "${pageContext.request.contextPath}/Menu/findlikeMenu",//请求的action路径  
