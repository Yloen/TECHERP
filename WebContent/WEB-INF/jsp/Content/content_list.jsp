...
<body>
	...
	<span class="c-gray en">&gt;</span> 类容管理 <span class="c-gray en">&gt;</span>
	类容列表 <a class="btn btn-success radius r"
	...
	<div class="page-container">
		<div class="text-c">
			...
			<form id="search_name">
			    <input type="text" class="input-text" value=2 style="width:0px" id="Status" name="Status">
		...
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l"><a href="javascript:;" onclick="delect_batch()"
				...
				onclick="member_add('添加类容','${pageContext.request.contextPath}/Content/Content_add','','600')"
  ...
  function member_del(obj, id) {
    layer.confirm('确认要删除吗？', function (index) {
      $.ajax({
        type: 'POST',
        url: '${pageContext.request.contextPath}/Content/Content_Recovery?ContentId='+id,
        dataType: 'json',
 ...
 function delect_batch(){ 
   ...
   //将数组元素连接起来转化为字符串
   var ContentId = arry.join(',');
   //输出到控制台
   console.log("选中值："+ ContentId);
 
     if (confirm("是否 删除?")){
       $.post ("${pageContext.request.contextPath}/Content/Content_RecoveryBatch", {"ContentId":ContentId},
       function (data){
      if (data>0) {
         alert("删除成功");
          }else {
